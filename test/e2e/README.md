...
test/e2e/run-all.sh --include-non-glm   # include non-zai/glm-5.3 providers
...
Container runs targeting providers other than `zai/glm-5.3` must call
`skip_unless_non_glm`. The project policy is to exercise containers only
against GLM-5.3 unless opted into explicitly with `--include-non-glm`.
...
| 30-run-basic | `airun "prompt"` against GLM-5.3 (requires --with-network) |
