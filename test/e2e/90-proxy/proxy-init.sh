#!/usr/bin/env bash
# `airun proxy init` creates ~/.airun/proxy.yaml and ~/.airun/users.json
# with 0600 permissions, seeds the yaml with a commented template, and makes
# users.json an empty JSON array.
...
users="$th/.airun/users.json"
assert_file_exists "$cfg"   "proxy.yaml created"
assert_file_exists "$users" "users.json created"
...
assert_eq "600" "$mode" "users.json permissions"
...
assert_contains "$users_content" "[]" "users.json starts empty"
