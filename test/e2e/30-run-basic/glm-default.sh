...
# the user's real ZAI_API_KEY, run one short prompt on GLM-5.3, expect exit 0.
#
...
ec=$?
assert_exit_code "0" "$ec" "airun GLM-5.3 run exits 0"
# Model output can be verbose and wrapped; we just check that the container
# reported provider=zai and model=glm-5.3 somewhere in its banner so we know
# routing worked.
assert_contains "$out" "provider=zai"   "airun banner reports provider=zai"
assert_contains "$out" "model=glm-5.3"  "airun banner reports model=glm-5.3"
